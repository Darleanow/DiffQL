...
bool same_optional(const std::optional<std::string> &a, const std::optional<std::string> &b)
{
...
int rename_score(const Column &old_col, const Column &new_col)
{
  ...
  if(old_col.type.base == new_col.type.base) score += 4;
  if(old_col.nullable == new_col.nullable) score += 1;
  if(old_col.auto_increment == new_col.auto_increment) score += 1;
  if(same_optional(old_col.default_value, new_col.default_value)) score += 1;

  if(!old_col.type.raw_type.empty() && !new_col.type.raw_type.empty() &&
     lower(old_col.type.raw_type) == lower(new_col.type.raw_type)) {
    score += 1;
  }
...
bool potentially_lossy_type_change(const Column &old_col, const Column &new_col)
{
  ...
  if(old_type == new_type) {
    ...
    if(old_type == CanonicalType::DECIMAL &&
       old_col.type.precision.has_value() && new_col.type.precision.has_value() &&
       *new_col.type.precision < *old_col.type.precision) {
      return true;
    }

    if(old_type == CanonicalType::DECIMAL &&
       old_col.type.scale.has_value() && new_col.type.scale.has_value() &&
                              ...
                              const std::vector<Table> &target)
{
  ...
  for(const Table &t : source) source_tables[table_key(t.schema, t.name)] = &t;
  for(const Table &t : target) target_tables[table_key(t.schema, t.name)] = &t;
  ...
  for(const auto &[key, table] : source_tables) {
    if(target_tables.find(key) == target_tables.end()) {
      ...
      result.warnings.push_back({"destructive", "DROP TABLE " + table->schema + "." + table->name});
    }
  }
  ...
  for(const auto &[key, old_table] : source_tables) {
    ...
    if(it_target == target_tables.end()) continue;
    ...
    for(const Column &c : old_table->columns) old_cols[column_key(c.name)] = &c;
    for(const Column &c : new_table->columns) new_cols[column_key(c.name)] = &c;
    ...
    for(const auto &[ck, c] : old_cols) {
      ...
      if(it_new == new_cols.end()) {
        ...
        if(lower(c->type.raw_type) != lower(nc->type.raw_type)) {
          ...
          if(risky) {
            result.warnings.push_back({
                "type_change",
                "Potentially lossy ALTER TYPE on " + old_table->schema + "." +
                    old_table->name + "." + c->name + " (" + c->type.raw_type + " -> " +
                    nc->type.raw_type + ")"
            });
          }
        }
    ...
    while(true) {
      ...
      if(best_score < 6 || best_i < 0 || best_j < 0) break;
      ...
      result.renamed_columns.push_back({
          ...
          added[static_cast<size_t>(best_j)]->name,
          best_score
      });
    }
    ...
    for(size_t i = 0; i < removed.size(); ++i) {
      if(removed_used[i]) continue;
      result.removed_columns.push_back({old_table->schema, old_table->name, removed[i]->name});
      result.warnings.push_back({
          "destructive",
          "DROP COLUMN " + old_table->schema + "." + old_table->name + "." + removed[i]->name
      });
    }

    for(size_t j = 0; j < added.size(); ++j) {
      if(added_used[j]) continue;
      result.added_columns.push_back({new_table->schema, new_table->name, added[j]->name});
    }

    std::unordered_map<std::string, std::string> old_checks;
    std::unordered_map<std::string, std::string> new_checks;

    for(const CheckConstraint &c : old_table->checks) old_checks[check_key(c.name)] = c.expression;
    for(const CheckConstraint &c : new_table->checks) new_checks[check_key(c.name)] = c.expression;
...
std::string diff_report_markdown(const SchemaDiffResult &result)
{
  ...
  for(const auto &t : result.added_tables) out << "  - + " << t.schema << "." << t.table << "\n";
  out << "- Removed tables: " << result.removed_tables.size() << "\n";
  for(const auto &t : result.removed_tables) out << "  - - " << t.schema << "." << t.table << "\n";
  out << "\n";
  ...
  for(const auto &c : result.added_columns) out << "  - + " << c.schema << "." << c.table << "." << c.column << "\n";
  out << "- Removed columns: " << result.removed_columns.size() << "\n";
  for(const auto &c : result.removed_columns) out << "  - - " << c.schema << "." << c.table << "." << c.column << "\n";
  out << "\n";
  ...
  for(const auto &c : result.type_changes) {
    ...
    if(c.potentially_lossy) out << " [RISK]";
    out << "\n";
  }
  out << "\n";
