    ...
    const std::vector<Table> &dest_schema)
{
  ...
  md << "# DiffQL Migration Report\n\n";
  md << "## Summary\n\n";
  md << "| Metric | Count |\n";
  md << "|--------|-------|\n";
  md << "| Tables changed | " << diff.table_diffs.size() << " |\n";
  md << "| Added | " << added << " |\n";
  md << "| Dropped | " << dropped << " |\n";
  md << "| Modified | " << modified << " |\n";
  md << "\n";

  if(!diff.table_diffs.empty()) {
    md << "## Changes\n\n";
    ...
    for(const auto &td : diff.table_diffs) {
      ...
      md << "### " << badge << " `" << td.table_name << "`";
      if(td.new_name.has_value())
        md << " → `" << *td.new_name << "`";
      md << "\n\n";
      md << "**Action:** " << action_str << "\n\n";

      if(td.action == DiffAction::MODIFIED) {
        if(!td.column_diffs.empty()) {
          md << "#### Columns\n\n";
          md << "| Column | Change |\n";
          md << "|--------|--------|\n";
          for(const auto &cd : td.column_diffs) {
            std::string change;
            switch(cd.action) {
              case DiffAction::ADDED:    change = "➕ Added"; break;
              case DiffAction::DROPPED:  change = "➖ Dropped"; break;
              case DiffAction::MODIFIED: change = "✏️ Modified"; break;
            }
            md << "| `" << cd.name << "` | " << change << " |\n";
          }
          md << "\n";
        ...
        if(td.pk_diff.has_value()) {
          md << "#### Primary Key\n\n";
          std::string change;
          switch(td.pk_diff->action) {
            case DiffAction::ADDED:    change = "➕ Added"; break;
            case DiffAction::DROPPED:  change = "➖ Dropped"; break;
            case DiffAction::MODIFIED: change = "✏️ Modified"; break;
          }
        ...
        if(!td.fk_diffs.empty()) {
          md << "#### Foreign Keys\n\n";
          md << "| Foreign Key | Change |\n";
          md << "|-------------|--------|\n";
          for(const auto &fk : td.fk_diffs) {
            std::string change;
            switch(fk.action) {
              case DiffAction::ADDED:    change = "➕ Added"; break;
              case DiffAction::DROPPED:  change = "➖ Dropped"; break;
        ...
        if(!td.index_diffs.empty()) {
          md << "#### Indexes\n\n";
          md << "| Index | Change |\n";
          md << "|-------|--------|\n";
          for(const auto &ix : td.index_diffs) {
            std::string change;
            switch(ix.action) {
              case DiffAction::ADDED:    change = "➕ Added"; break;
              case DiffAction::DROPPED:  change = "➖ Dropped"; break;
        ...
        if(!td.check_diffs.empty()) {
          md << "#### Check Constraints\n\n";
          md << "| Constraint | Change |\n";
          md << "|------------|--------|\n";
          for(const auto &ck : td.check_diffs) {
            std::string change;
            switch(ck.action) {
...
void run_tui_mode(SchemaLoader source_loader, SchemaLoader target_loader)
{
  ...
  std::thread worker([&] {
    ...
    if(diff_result.table_diffs.empty()) {
      phase.store(Phase::NO_DIFF);
    } else {
  ...
  auto renderer = Renderer([&] {
    ...
    if(p == Phase::NO_DIFF) {
      auto content = vbox({
          text("") | flex,
          vbox({
              text(" No differences found ") | bold | color(Color::GreenLight) | hcenter,
              text(""),
              text(" Schemas are identical ") | color(Color::GrayLight) | hcenter,
          }) | center | flex,
          text("") | flex,
      });

      auto footer = hbox({
          text(" Press Enter to exit ") | color(Color::GrayLight),
      });

      return vbox({
          hbox({text(" DiffQL ") | bold | color(Color::CyanLight), filler()}),
          separatorLight(),
          content | flex,
          separatorLight(),
          footer,
    ...
    if(p == Phase::GENERATING) {
      ...
      if(gen_ok) {
        ...
        std::string md_path = saved_path;
        auto dot_pos = md_path.rfind('.');
        ...
        auto confirm_box = vbox({
            ...
            separator(),
            text(""),
            text("  Script: " + saved_path) | color(Color::GreenLight),
            text("  Report: " + md_path) | color(Color::CyanLight),
            text(""),
            separator(),
            hbox({
                text("  "),
    ...
    if(p == Phase::GENERATING) {
      ...
      if(event == Event::Return) {
        ...
        if(out) {
          ...
          // Generate markdown report alongside
          std::string md_path = gen_path;
          auto dot_pos = md_path.rfind('.');
          if(dot_pos != std::string::npos)
            md_path = md_path.substr(0, dot_pos) + ".md";
          else
          ...
          std::string report = generate_markdown_report(
              diff_result, origin_schema_result, dest_schema_result);
